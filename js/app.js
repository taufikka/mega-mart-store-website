...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="height border-0 card-overflow">
      <div>
    <img class="product-image" src=${image}></img>
      </div>
      <h3 class='mt-2'>${product.title}</h3>
      <p>Category: ${product.category}</p>
      <p class="fw-bolder">Average-rating:  ${product.rating.rate}</p>
      <p class="fw-bolder">Rated-by:  ${product.rating.count} person</p>
      <h2>Price: $ ${product.price}</h2>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="btn btn-dark">add to cart</button>
